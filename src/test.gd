...
## Sigilo automático al entrar en zonas marcadas como hide_spot.
...
func _physics_process(_delta: float) -> void:
	...
	# Sigilo automático al estar en hide_spot
	if _in_hide_spot and current_state not in [State.DEAD, State.INTANGIBLE]:
		if current_state != State.STEALTH:
			current_state = State.STEALTH
			_particles.emitting = true
			get_tree().call_group("enemies", "on_player_stealthed", true)
	elif current_state == State.STEALTH and not _in_hide_spot:
		_exit_stealth()
...
# ─── Animaciones ───────────────────────────────────────────────────────────────
func _update_animation(dir: Vector2, is_sprinting: bool) -> void:
	if dir.x != 0.0:
	...
	if dir == Vector2.ZERO:
		prefix = "idle"
	elif current_state == State.STEALTH:
		prefix = "stealth"
	elif is_sprinting:
		prefix = "run"
	else:
