...
func _ready() -> void:
	...
	alerted.connect(func(): AudioManager.play_sfx("fox_bark", global_position, 0.0, 0.9, 1.1, 800.0))
	sniffed.connect(func(): AudioManager.play_sfx("fox_sniff", global_position, -4.0, 1.0, 1.2, 300.0))
...
func _physics_process(_delta: float) -> void:
	match current_state:
		...
		State.INVESTIGATE:
			...
			# Emitir olfateo aleatorio mientras investiga
			_sniff_timer -= _delta
			if _sniff_timer <= 0.0:
				sniffed.emit()
				_sniff_timer = randf_range(0.8, 2.0)
